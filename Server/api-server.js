	...
	mDb.query("SELECT * FROM `somabell_user` WHERE `token`='"+req.query.token+"'",function(err,rows,field){
		...
		if(rows[0]){
			rDb.get("user_"+rows[0].id,function(err,value){
				mDb.query("SELECT * FROM `somabell_store_store` WHERE `id`="+JSON.parse(value).sid,function(err,rows,field){
					if(err){
						res.send({"result_code": -98, "result_msg": "Database Error"});
						return;
					}
					...
					rDb.llen("list_"+yyyy+mm+dd+"_"+JSON.parse(value).sid,function(err,maxPeople){
						rDb.llen("list_"+JSON.parse(value).sid,function(err,extraPeople){
							var store = JSON.parse(rows[0].json);
							store["current_num"] = maxPeople-extraPeople+1;
							res.send({"result_code": 0, "result_msg": "git", "ticket": JSON.parse(value), "store": store});
						});
					});
				});
			});
...
app.get("/user/auth/gcm",function(req,res){
	mDb.query("SELECT * FROM `somabell_user` WHERE `token`='"+req.query.token+"'",function(err,rows,field){
		if(err){
			res.send({"result_code": -99, "result_msg": "Database Error"});
			return;
		...
		if(rows[0]){
			mDb.query("UPDATE `somabell_user` SET `gcm`='"+req.query.gcm+"' WHERE `token`='"+req.query.token+"'",function(err,rows,field){
				if(err){
					res.send({"result_code": -98, "result_msg": "Database Error"});
					return;
				}
...
app.get("/user/beacon/get",function(req,res){
	mDb.query("SELECT * FROM `somabell_store_beacon` WHERE `major`="+req.query.major+" AND `minor`="+req.query.minor,function(err,rows,field){
		if(err){
			throw err;
			res.send({"result_code": -99, "result_msg": "Database Error"});
			return;
		...
		if(rows[0])
			res.send({"result_code": 0,"result_msg": "success", "sid": rows[0].owner});
		else
			res.send({"result_code": -1, "result_msg": "fail"});
	});
});

app.get("/user/store/put",function(req,res){
	mDb.query("SELECT * FROM `somabell_user` WHERE `token`='"+req.query.token+"'",function(err,rows,field){
		if(err){
			res.send({"result_code": -99, "result_msg": "Database Error"});
			return;
		...
		if(rows[0]){
			mDb.query("SELECT * FROM `somabell_store_store` WHERE `id`="+req.query.sid,function(err,rows2,field){
				if(err){
					res.send({"result_code": -98, "result_msg": "Database Error"});
					return;
				}
				if(rows[0]){
					...
					rDb.rpush("list_"+yyyy+mm+dd+"_"+req.query.sid,JSON.stringify({"uid": rows[0].id,"num": req.query.num, "mdn": rows[0].mdn, "createTime": Math.floor(Date.now()/1000), "status": 0}));
					rDb.llen("list_"+yyyy+mm+dd+"_"+req.query.sid,function(err,value){
						rDb.set("user_"+rows[0].id,JSON.stringify({"sid": req.query.sid,"ticket": value,"uid": req.query.uid,"num": req.query.num, "mdn": rows[0].mdn, "createTime": Math.floor(Date.now()/1000)}));
						rDb.rpush("list_"+req.query.sid,JSON.stringify({"ticket": value,"uid": rows[0].id,"num": req.query.num, "gcm": rows[0].gcm, "mdn": rows[0].mdn, "createTime": Math.floor(Date.now()/1000)}));
						rDb.publish("calc",JSON.stringify(rows2[0]));
						res.send({"result_code": 0, "result_msg": "success"});
					});
				}
			});
					...
					mDb.query("UPDATE `somabell_store_email` SET `password`='"+pass+"', `mdn`='"+req.query.mdn+"' WHERE `token`='"+req.query.token+"'",function(err,rows,field){
						if(err){
							...
							mDb.query("SELECT * FROM `somabell_store_email` WHERE `token`='"+req.query.token+"'",function(err,rows,field){
								...
								res.send({"result_code": 1, "result_msg": "yeah", "token": rows[0].token, "uid": rows[0].id});
							});
						}
					});
				}
...
app.get("/store/auth/gcm",function(req,res){
	mDb.query("SELECT * FROM `somabell_store_email` WHERE `token`='"+req.query.token+"'",function(err,rows,field){
		if(err){
			res.send({"result_code": -99, "result_msg": "Database Error"});
			return;
		...
		if(rows[0]){
			mDb.query("UPDATE `somabell_store_email` SET `gcm`='"+req.query.gcm+"' WHERE `token`='"+req.query.token+"'",function(err,rows,field){
				if(err){
					res.send({"result_code": -98, "result_msg": "Database Error"});
					return;
				}
...
app.get("/store/store/log",function(req,res){
	mDb.query("SELECT * FROM `somabell_store_email` WHERE `token`='"+req.query.token+"'",function(err,rows,filed){
		if(err){
			res.send({"result_code": -99, "result_msg": "Database Error"});
			return;
		...
		if(rows[0]){
			mDb.query("SELECT * FROM `somabell_store_store` WHERE `id`="+req.query.sid+" AND `owner`="+rows[0].id,function(err,rows,field){
				if(err){
					...
					rDb.lrange("list_"+date+"_"+req.query.sid,0,-1,function(err,value){
						for(var i=0;i<value.length;i++){
							value[i] = JSON.parse(value[i]);
						}

						res.send({"result_code": 0, "result_msg": "success", "sid": req.query.sid, "tickets": value});
					});
				}
			});
		}
			...
			mDb.query("SELECT * FROM `somabell_store_store` WHERE `id`="+req.query.sid+" AND `owner`="+rows[0].id,function(err,rows,field){
				if(err){
					...
					mDb.query("SELECT * FROM `somabell_user` WHERE id="+req.query.uid,function(err,rows,field){
						...
						rDb.rpush("list_"+yyyy+mm+dd+"_"+req.query.sid,JSON.stringify({"uid": req.query.uid,"num": req.query.num, "mdn": rows[0].mdn, "createTime": Math.floor(Date.now()/1000), "status": 0}));
						rDb.llen("list_"+yyyy+mm+dd+"_"+req.query.sid,function(err,value){
							...
							rDb.rpush("list_"+req.query.sid,JSON.stringify({"ticket": value,"uid": req.query.uid,"num": req.query.num, "gcm": rows[0].gcm, "mdn": rows[0].mdn, "createTime": Math.floor(Date.now()/1000)}));
							rDb.publish("calc",JSON.stringify(rows[0]));
							res.send({"result_code": 0, "result_msg": "success"});
						});
					});
...
app.get("/store/store/pop",function(req,res){
	mDb.query("SELECT * FROM `somabell_store_email` WHERE `token`='"+req.query.token+"'",function(err,rows,filed){
		if(err){
			res.send({"result_code": -99, "result_msg": "Database Error"});
			return;
		...
		if(rows[0]){
			mDb.query("SELECT * FROM `somabell_store_store` WHERE `id`="+req.query.sid+" AND `owner`="+rows[0].id,function(err,rows,field){
				if(err){
					...
					rDb.lindex("list_"+req.query.sid,req.query.index,function(err,value){
						if(value){
							var today = new Date();
							var dd = today.getDate();
							var mm = today.getMonth()+1;
							...
							rDb.lrem("list_"+req.query.sid,0,value,function(err){
								rDb.lindex("list_"+yyyy+mm+dd+"_"+req.query.sid,JSON.parse(value).ticket-1,function(err,value2){
									var dat = JSON.parse(value2);

									dat.status = 1;

									rDb.lset("list_"+yyyy+mm+dd+"_"+req.query.sid,JSON.parse(value).ticket-1,JSON.stringify(dat),function(err){
										rDb.publish("calc",JSON.stringify(rows[0]));
										res.send({"result_code": 0, "result_msg": "success"});
									});
								});
...
app.get("/store/store/cancle",function(req,res){
	mDb.query("SELECT * FROM `somabell_store_email` WHERE `token`='"+req.query.token+"'",function(err,rows,filed){
		if(err){
			res.send({"result_code": -99, "result_msg": "Database Error"});
			return;
